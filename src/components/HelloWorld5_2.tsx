...
const HelloWorld5_2: React.FC<Props> = ({ name = "John" }) => {
  ...
  useEffect(() => {
    window.alert(
      `Hello ${state.nameToGreet}, be aware that you have clicked ${state.count} time(s).`
    );
    //if you remove the line below, you will get a warning because only count and not name is set as condition// eslint-disable-next-line
  }, [state.count]);
  return (
